...
////	dist_to_compare = point_intersect - actor_pose.Pos();
////	dist_to_compare.Z(0.0); // will be compared to planar radius
////
////	std::cout << debug_current_actor_name << "'s center: " <<actor_pose.Pos() << "\tBC's center: " << actor_bound.GetCenter() << std::endl;
//
////	if ( dist_to_compare.Length() <= actor_bound.GetRadius() ) {
//
...
//	if ( object_name == "table1" ) {
//		std::cout << "GetClosestPoints()\t" << object_name << "'s BB point of intersection: " << point_intersect;
//	}
//#endif
//
...
//	std::cout << "\n\nBOUND - actor & object | actor pos: " << actor_pose.Pos() << "\tintersection: " << actor_pose_shifted.Pos() << std::endl;
//	std::cout << "BOUND - actor & object |" << object_name << "'s pos: " << object_pose.Pos() << "\tintersection: " << point_intersect << std::endl;
//	#endif
//
//	return ( std::make_tuple(actor_pose_shifted, point_intersect) );
//
		...
		const std::string &object_name /* debug only */) const
{
	...
	// above commented -> instead isWithin method created
	if ( actor_circle.doesContain(point_intersect) ) {
	...
	// intersection of the actor's circle
	std::tie(std::ignore, actor_pose_shifted.Pos()) = actor_circle.getIntersection(point_intersect);
